...
class ZCRMModule(object):
    ...
    def create_records(self, record_ins_list):
        ...
        return MassEntityAPIHandler.get_instance(self).create_records(
            record_ins_list)
    ...
    def upsert_records(self, record_ins_list):
        ...
        return MassEntityAPIHandler.get_instance(self).upsert_records(
            record_ins_list)
    ...
    def get_customview(self, customview_id):
        ...
        return ModuleAPIHandler.get_instance(self).get_customview(
            customview_id)
    ...
    def get_relatedlist(self, relatedlist_id):
        ...
        return ModuleAPIHandler.get_instance(self).get_relatedlist(
            relatedlist_id)
    ...
    def update_customview(self, customview_instance):
        ...
        return ModuleAPIHandler.get_instance(self).update_customview(
            customview_instance)
...
class ZCRMRecord(object):
    ...
    def get(self):
        if self.entity_id is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception('Record_Get',
    ...
    def upload_attachment(self, file_path):
        if file_path is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception('Upload_Attachment',
                                       "file path must be given",
                                       'FILEPATH NOT PROVIDED', "FILEPATH")

        return ZCRMModuleRelation.get_instance(
            self, "Attachments").upload_attachment(file_path)
    ...
    def upload_link_as_attachment(self, link_url):
        if link_url is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception('Upload_Link_As_Attachment',
                                       "link URL must be given",
                                       'URL_LINK NOT PROVIDED', "URL_LINK")

        return ZCRMModuleRelation.get_instance(
            self, "Attachments").upload_link_as_attachment(link_url)
    ...
    def download_attachment(self, attachment_id):
        if attachment_id is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception('Download_Attachment',
                                       "attachment id must be given",
                                       'ID DOES NOT PROVIDED', "ID")

        return ZCRMModuleRelation.get_instance(
            self, "Attachments").download_attachment(attachment_id)
    ...
    def delete_attachment(self, attachment_id):
        if attachment_id is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception('Delete_Attachment',
                                       "attachment id must be given",
                                       'ID DOES NOT PROVIDED', "ID")
    ...
    def add_relation(self, junction_record):
        if junction_record is None:
            ...
            CommonUtil.raise_exception(
                'add_relation', "junction_record must be given",
                'JUNCTION RECORD NOT PROVIDED', "JUNCTION RECORD")

        return ZCRMModuleRelation.get_instance(self,
                                               junction_record).add_relation()
    ...
    def remove_relation(self, junction_record):
        if junction_record is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception(
                'remove_relation', "junction_record must be given",
                'JUNCTION RECORD NOT PROVIDED', "JUNCTION RECORD")

        return ZCRMModuleRelation.get_instance(
            self, junction_record).remove_relation()
    ...
    def add_note(self, note_ins):
        if note_ins is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception(
                'add_note', "note instance must be given",
                'NOTE INSTANCE NOT PROVIDED', "NOTE INSTANCE")

        return ZCRMModuleRelation.get_instance(self,
                                               "Notes").add_note(note_ins)
    ...
    def update_note(self, note_ins):
        if note_ins is None or note_ins.id is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception(
                'update_note', "note instance and note id must be given",
                'NOTE INSTANCE OR NOTE ID NOT PROVIDED', "NOTE INSTANCE OR ID")

        return ZCRMModuleRelation.get_instance(self,
                                               "Notes").update_note(note_ins)
    ...
    def delete_note(self, note_ins):
        if note_ins is None or note_ins.id is None:
            try:
                ...
                from Utility import CommonUtil
            CommonUtil.raise_exception(
                'delete_note', "note instance and note id must be given",
                'NOTE INSTANCE OR NOTE ID NOT PROVIDED', "NOTE INSTANCE OR ID")

        return ZCRMModuleRelation.get_instance(self,
                                               "Notes").delete_note(note_ins)

    def get_notes(self, sort_by=None, sort_order=None, page=1, per_page=20):
        return ZCRMModuleRelation.get_instance(self, "Notes").get_notes(
    ...
    def get_relatedlist_records(self,
        ...
        return ZCRMModuleRelation.get_instance(
            self, relatedlist_api_name).get_records(sort_by, sort_order, page,
                                                    per_page)
...
class ZCRMUser(object):
    ...
    defaultKeys = [
        "Currency", "Modified_Time", "created_time", "territories",
        "reporting_to", "Isonline", "created_by", "Modified_By", "country",
        "id", "name", "role", "customize_info", "city", "signature",
        "name_format", "language", "locale", "personal_account",
        "default_tab_group", "alias", "street", "theme", "state",
        "country_locale", "fax", "first_name", "email", "zip",
        "decimal_separator", "website", "time_format", "profile", "mobile",
        "last_name", "time_zone", "zuid", "confirm", "full_name", "phone",
        "dob", "date_format", "status"
    ]
...
class ZCRMModuleRelatedList(object):
    ...
    def set_relatedlist_properties(self, relatedlist_prop):
        ...
        self.is_visible = bool(relatedlist_prop['visible']
                               ) if 'visible' in relatedlist_prop else None

        return self


class ZCRMModuleRelation(object):
    def __init__(self, parentmodule_apiname_or_parentrecord,
                 related_list_apiname_or_junction_record):
        ...
        if isinstance(related_list_apiname_or_junction_record,
                      ZCRMJunctionRecord):
    ...
    @staticmethod
    def get_instance(parentmodule_apiname_or_parentrecord,
                     related_list_apiname_or_junction_record):

        return ZCRMModuleRelation(parentmodule_apiname_or_parentrecord,
                                  related_list_apiname_or_junction_record)
