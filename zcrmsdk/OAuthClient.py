...
class ZohoOAuth(object):
    ...
    def initialize(config_dict=None):
        try:
            ...
            if (ZohoOAuthConstants.TOKEN_PERSISTENCE_PATH not in ZohoOAuth.
                    configProperties or ZohoOAuth.configProperties[
                        ZohoOAuthConstants.TOKEN_PERSISTENCE_PATH] == ""):

                if (ZohoOAuthConstants.DATABASE_PORT not in ZohoOAuth.
                        configProperties or ZohoOAuth.configProperties[
                            ZohoOAuthConstants.DATABASE_PORT] == ""):
                    ZohoOAuth.configProperties[ZohoOAuthConstants.
                                               DATABASE_PORT] = "3306"

                if (ZohoOAuthConstants.DATABASE_USERNAME not in ZohoOAuth.
                        configProperties or ZohoOAuth.configProperties[
                            ZohoOAuthConstants.DATABASE_USERNAME] == ""):
                    ZohoOAuth.configProperties[ZohoOAuthConstants.
                                               DATABASE_USERNAME] = "root"

                if (ZohoOAuthConstants.DATABASE_PASSWORD not in ZohoOAuth.
                        configProperties or ZohoOAuth.configProperties[
    ...
    def set_config_values(config_dict):
        config_keys = [
            ...
            ZohoOAuthConstants.REDIRECT_URL, ZohoOAuthConstants.ACCESS_TYPE,
        ...
        if (ZohoOAuthConstants.ACCESS_TYPE not in config_dict
                or config_dict[ZohoOAuthConstants.ACCESS_TYPE] is None):
            ZohoOAuth.configProperties[ZohoOAuthConstants.
                                       ACCESS_TYPE] = "offline"

        if (ZohoOAuthConstants.IAM_URL not in config_dict
                or config_dict[ZohoOAuthConstants.IAM_URL] == ""):
            ZohoOAuth.configProperties[ZohoOAuthConstants.
    ...
    @staticmethod
    def get_grant_url():
        return (ZohoOAuth.configProperties[ZohoOAuthConstants.IAM_URL] +
                "/oauth/v2/auth")
    ...
    def get_client_instance():
        ...
        if (oauth_client_ins is None):
            raise ZohoOAuthException(
                'ZohoOAuth.initialize() must be called before this')
...
class ZohoOAuthClient(object):
    ...
    def get_instance(param=None):
        if (param is not None and ZohoOAuthClient.oAuthClientIns is None):
    ...
    def get_access_token(self, userEmail):
        try:
            ...
            try:
                ...
                OAuthLogger.add_log("Access token expired hence refreshing",
                                    logging.INFO, e)
                oAuthTokens = self.refresh_access_token(
                    oAuthTokens.refreshToken, userEmail)

                return oAuthTokens.accessToken
        except Exception as ex:
            OAuthLogger.add_log(
                "Exception occured while fetching oauthtoken from db",
                logging.ERROR, ex)
            raise ex

    def generate_access_token_from_refresh_token(self, refreshToken,
                                                 userEmail):
