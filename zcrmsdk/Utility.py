...
class HTTPConnector(object):
    ...
    def get_instance(url, params, headers, body, method, apiKey, is_bulk_req):
        return HTTPConnector(url, params, headers, body, method, apiKey,
                             is_bulk_req)

    def __init__(self, url, params, headers, body, method, apiKey,
                 is_bulk_req):
        '''
        Constructor
        '''
...
class APIConstants(object):
    ...
    FAULTY_RESPONSE_CODES = [
        RESPONSECODE_NO_CONTENT, RESPONSECODE_NOT_FOUND,
...
class ZCRMConfigUtil(object):
    ...
    def set_config_values(config_dict):
        config_keys = [
            APIConstants.CURRENT_USER_EMAIL, ZohoOAuthConstants.SANDBOX,
            APIConstants.API_BASEURL, APIConstants.API_VERSION,
            APIConstants.APPLICATION_LOGFILE_PATH
        ]

        if (ZohoOAuthConstants.SANDBOX not in config_dict
                or config_dict[ZohoOAuthConstants.SANDBOX] == ""):
            ZCRMConfigUtil.config_prop_dict[ZohoOAuthConstants.
                                            SANDBOX] = "false"

        if (APIConstants.API_BASEURL not in config_dict
                or config_dict[APIConstants.API_BASEURL] == ""):
            ZCRMConfigUtil.config_prop_dict[APIConstants.
                                            API_BASEURL] = "www.zohoapis.com"

        if (APIConstants.API_VERSION not in config_dict
                or config_dict[APIConstants.API_VERSION] == ""):
    ...
    def get_access_token(self):
        ...
        if (userEmail is None and
            ...
            raise ZCRMException(
                'fetching current user email', 400,
...
class CommonUtil(object):
    ...
    def raise_exception(url, message, details, content=None):
        zcrm_exception = ZCRMException(
            url, APIConstants.RESPONSECODE_INVALID_INPUT, message,
            APIConstants.STATUS_ERROR, details, content)
        import logging
        try:
