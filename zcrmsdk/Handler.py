...
class EntityAPIHandler(APIHandler):
    ...
    def get_record(self):
        try:
            ...
            self.set_record_properties(
                apiResponse.response_json[APIConstants.DATA][0])
    ...
    def create_record(self):
        try:
            ...
            try:
                ...
                from Utility import CommonUtil
            handler_ins.request_body = CommonUtil.create_api_supported_input_json(
                input_json, APIConstants.DATA)
            apiResponse = APIRequest(handler_ins).get_api_response()
            reponseDetails = apiResponse.response_json[APIConstants.
                                                       DATA][0]['details']
            ...
            try:
                ...
                from Operations import ZCRMUser
            self.zcrmrecord.created_by = ZCRMUser.get_instance(
    ...
    def update_record(self):
        try:
            ...
            reponseDetails = api_response.response_json[APIConstants.
                                                        DATA][0]['details']
            self.zcrmrecord.entity_id = reponseDetails['id']
            self.zcrmrecord.created_time = reponseDetails['Created_Time']
            ...
            self.zcrmrecord.modified_by = ZCRMUser.get_instance(
    ...
    def convert_record(self, potential_record, assign_to_user):
        try:
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.zcrmrecord.module_api_name + \
                "/"+str(self.zcrmrecord.entity_id)+"/actions/convert"
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            input_json = dict()
            ...
            if potential_record is not None:
                input_json['Deals'] = self.get_instance(
                    potential_record).get_zcrmrecord_as_json()
            ...
            if APIConstants.CONTACTS in convertedIdsJson and convertedIdsJson[
                    APIConstants.CONTACTS] is not None:
                converted_dict[APIConstants.CONTACTS] = convertedIdsJson[
                    APIConstants.CONTACTS]

            if APIConstants.ACCOUNTS in convertedIdsJson and convertedIdsJson[
                    APIConstants.ACCOUNTS] is not None:
                converted_dict[APIConstants.ACCOUNTS] = convertedIdsJson[
                    APIConstants.ACCOUNTS]

            if APIConstants.DEALS in convertedIdsJson and convertedIdsJson[
                    APIConstants.DEALS] is not None:
                converted_dict[APIConstants.DEALS] = convertedIdsJson[
                    APIConstants.DEALS]
    ...
    def upload_photo(self, file_path):
        try:
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.zcrmrecord.module_api_name + \
                "/"+str(self.zcrmrecord.entity_id)+"/photo"
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
    ...
    def get_zcrmrecord_as_json(self):
        ...
        if len(self.zcrmrecord.line_items) > 0:
            record_json["Product_Details"] = self.get_line_item_json(
                self.zcrmrecord.line_items)
        ...
        if len(self.zcrmrecord.price_details) > 0:
            record_json[
                "Pricing_Details"] = self.get_price_details_as_jsonarray()
    ...
    def get_price_details_as_jsonarray(self):
        ...
        for priceBookPricingIns in priceDetailsList:
            priceDetailsArr.append(
                self.get_zcrmprice_detail_as_json(priceBookPricingIns))
    ...
    def get_participants_as_jsonarray(self):
        ...
        for participantIns in participantsList:
            participantsArr.append(
                self.get_zcrmparticipant_as_json(participantIns))
    ...
    def get_line_item_json(self, lineItemsArray):
        ...
        for lineItem in lineItemsArray:
            ...
            if lineItem.quantity is None:
                raise ZCRMException(self.request_url_path,
                                    APIConstants.RESPONSECODE_BAD_REQUEST,
                                    "Mandatory Field 'quantity' is missing.",
                                    None)
        ...
        for key in responseDict:
            ...
            if (key == "id"):
                ...
                if ("id" in value):
                    ...
                    lookupRecord.lookup_label = value if (
                        "name" in value) else None
    ...
    def set_price_details(self, priceDetails):
        for priceDetail in priceDetails:
            self.zcrmrecord.price_details.append(
                self.get_zcrm_pricebook_pricing(priceDetail))
    ...
    def set_participants(self, participants):
        for participant in participants:
            self.zcrmrecord.participants.append(
                self.get_zcrmparticipant(participant))
    ...
    def set_inventory_line_items(self, lineItems):
        for lineItem in lineItems:
            self.zcrmrecord.line_items.append(
                self.get_zcrminventory_line_item(lineItem))
    ...
    def get_zcrminventory_line_item(self, lineItemDetails):
        ...
        lineItemInstance = ZCRMInventoryLineItem.get_instance(
            lineItemDetails["id"])
        ...
        lineItemInstance.total_after_discount = float(
            lineItemDetails["total_after_discount"])
...
class RelatedListAPIHandler(APIHandler):
    ...
    def get_instance(parent_record, related_list_or_junction_record):
        return RelatedListAPIHandler(parent_record,
                                     related_list_or_junction_record)

    def get_records(self, sort_by_field, sort_order, page, per_page):
        try:
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.parent_record.module_api_name+"/" + \
                str(self.parent_record.entity_id) + \
                "/"+self.related_lists.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
            handler_ins.request_api_key = APIConstants.DATA
            ...
            for record_json in records_json:
                record_ins = ZCRMRecord.get_instance(
                    self.related_lists.api_name, record_json['id'])
                EntityAPIHandler.get_instance(
                    record_ins).set_record_properties(record_json)
    ...
    def add_relation(self):
        try:
            ...
            handler_ins.request_url_path = self.parent_record.module_api_name+"/" + \
                str(self.parent_record.entity_id)+"/" + \
                self.junction_record.api_name+"/"+str(self.junction_record.id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.request_api_key = APIConstants.DATA
            ...
            try:
                ...
                from Utility import CommonUtil
            handler_ins.request_body = CommonUtil.create_api_supported_input_json(
                junction_record_data, APIConstants.DATA)
    ...
    def add_note(self, zcrm_note_ins):
        try:
            ...
            try:
                ...
                from Utility import CommonUtil
            handler_ins.request_body = CommonUtil.create_api_supported_input_json(
                self.get_zcrmnote_as_json(zcrm_note_ins), APIConstants.DATA)
            api_response = APIRequest(handler_ins).get_api_response()
            details = api_response.response_json[APIConstants.DATA][0][
    ...
    def update_note(self, zcrm_note_ins):
        try:
            ...
            handler_ins.request_url_path = self.parent_record.module_api_name+"/" + \
                str(self.parent_record.entity_id)+"/" + \
                self.related_lists.api_name+"/"+str(zcrm_note_ins.id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.request_api_key = APIConstants.DATA
            try:
                from .Utility import CommonUtil
    ...
    def get_attachments(self, page, per_page):
        try:
            ...
            handler_ins.add_param('per_page', per_page)
            bulk_api_res = APIRequest(handler_ins).get_bulk_api_response()
            attachments_json_arr = bulk_api_res.response_json[APIConstants.
                                                              DATA]
            attachment_ins_arr = list()

            for attachment_json in attachments_json_arr:
                attachment_ins_arr.append(
                    self.get_zcrm_attachment(attachment_json))
    ...
    def get_zcrm_note(self, note_details, zcrm_note_ins):
        if zcrm_note_ins is None:
            try:
                ...
                from Operations import ZCRMNote
            zcrm_note_ins = ZCRMNote.get_instance(self.parent_record,
                                                  note_details['id'])
        ...
        if 'Owner' in note_details:
            zcrm_note_ins.owner = ZCRMUser.get_instance(
                note_details['Owner']['id'], note_details['Owner']['name'])
        zcrm_note_ins.created_by = ZCRMUser.get_instance(
            note_details['Created_By']['id'],
            note_details['Created_By']['name'])
        zcrm_note_ins.modified_by = ZCRMUser.get_instance(
            note_details['Modified_By']['id'],
            note_details['Modified_By']['name'])
        ...
        if '$attachments' in note_details and note_details[
            ...
            for attachment_details in attachment_arr:
                attachment_ins_arr.append(
                    self.get_zcrm_attachment(attachment_details))
    ...
    def get_zcrm_attachment(self, attachment_details):
        try:
            ...
            from Operations import ZCRMAttachment, ZCRMUser
        attachment_ins = ZCRMAttachment.get_instance(self.parent_record,
        ...
        attachment_ins.owner = ZCRMUser.get_instance(
            attachment_details["Owner"]["id"],
            attachment_details["Owner"]["name"])
        attachment_ins.created_by = ZCRMUser.get_instance(
            attachment_details["Created_By"]["id"],
            attachment_details["Created_By"]["name"])
        attachment_ins.modified_by = ZCRMUser.get_instance(
            attachment_details["Modified_By"]["id"],
            attachment_details["Modified_By"]["name"])
    ...
    def upload_attachment(self, file_path):
        try:
            ...
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.DATA
            api_response_ins = APIRequest(handler_ins).upload_attachment(
                file_path)
            details = api_response_ins.response_json[APIConstants.
                                                     DATA][0]['details']
            try:
                from .Operations import ZCRMAttachment
            except ImportError:
                from Operations import ZCRMAttachment
            attachment_ins = ZCRMAttachment.get_instance(
                self.parent_record, details['id'])
    ...
    def upload_link_as_attachment(self, link_url):
        try:
            ...
            api_response_ins = APIRequest(
                handler_ins).upload_link_as_attachment()
            details = api_response_ins.response_json[APIConstants.
                                                     DATA][0]['details']
            try:
                from .Operations import ZCRMAttachment
    ...
    def download_attachment(self, attachment_id):
        try:
            ...
            handler_ins.request_url_path = self.parent_record.module_api_name+"/" + \
                str(self.parent_record.entity_id)+"/" + \
                self.related_lists.api_name+"/"+str(attachment_id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
...
class MassEntityAPIHandler(APIHandler):
    ...
    def get_records(self, cvid, sort_by, sort_order, page, per_page):
        try:
            ...
            for record_data in data_arr:
                zcrm_record = ZCRMRecord.get_instance(
                    self.module_instance.api_name, record_data['id'])
                EntityAPIHandler.get_instance(
                    zcrm_record).set_record_properties(record_data)
        ...
        try:
            ...
            for record_ins in record_ins_list:
                if record_ins.entity_id is None:
                    data_array.append(
                        EntityAPIHandler.get_instance(record_ins).
                        get_zcrmrecord_as_json())
                else:
                    CommonUtil.raise_exception(
                        'Records_Create', "record id must be None",
                        'RECORD ID PROVIDED', "RECORD ID")
            request_json = dict()
            ...
            for i in range(0, length):
                ...
                if entity_response_ins.status == APIConstants.STATUS_SUCCESS:
                    ...
                    EntityAPIHandler.get_instance(
                        new_record).set_record_properties(
                            record_create_details)
    ...
    def upsert_records(self, record_ins_list):
        try:
            ...
            for record_ins in record_ins_list:
                record_json = EntityAPIHandler.get_instance(
                    record_ins).get_zcrmrecord_as_json()
    ...
    def update_mass_records(self, entityid_list, field_api_name, value):
        try:
            ...
            if len(entityid_list) > 100:
                CommonUtil.raise_exception(
                    'Records_Mass_Update',
                    "entity id count must be less than or equals to 100",
                    'MORE RECORDS PROVIDED', "MORE RECORDS")
            handler_ins = APIHandler()
            handler_ins.request_url_path = self.module_instance.api_name
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.request_api_key = APIConstants.DATA
            handler_ins.request_body = self.construct_json_for_massupdate(
            ...
            for i in range(0, length):
                ...
                if entity_response_ins.status == APIConstants.STATUS_SUCCESS:
                    ...
                    updated_record = ZCRMRecord.get_instance(
                        self.module_instance.api_name,
                        record_update_details['id'])
                    EntityAPIHandler.get_instance(
                        updated_record).set_record_properties(
                            record_update_details)
    ...
    def delete_records(self, entityid_list):
        try:
            ...
            handler_ins.request_api_key = APIConstants.DATA
            ids_as_string = ','.join(
                str(entity_id) for entity_id in entityid_list)
            ...
            for i in range(0, length):
                ...
                deleted_record = ZCRMRecord.get_instance(
                    self.module_instance.api_name, record_delete_details['id'])
    ...
    def get_deleted_records(self, trashed_type):
        try:
            ...
            for record_data in data_arr:
                trash_record = ZCRMTrashRecord.get_instance(
                    record_data['type'], record_data['id'])
    ...
    def set_trash_record_properties(self, trash_record_ins, trash_record_prop):
        ...
        if 'created_by' in trash_record_prop and trash_record_prop[
                'created_by'] is not None:
            trash_record_ins.created_by = ZCRMUser.get_instance(
                trash_record_prop['created_by']['id'],
                trash_record_prop['created_by']['name'])

        if 'deleted_by' in trash_record_prop and trash_record_prop[
                'deleted_by'] is not None:
            trash_record_ins.deleted_by = ZCRMUser.get_instance(
                trash_record_prop['deleted_by']['id'],
                trash_record_prop['deleted_by']['name'])
        trash_record_ins.deleted_time = trash_record_prop['deleted_time']

    def construct_json_for_massupdate(self, entityid_list, field_api_name,
                                      field_value):
        input_json_arr = list()
...
class ModuleAPIHandler(APIHandler):
        ...
        try:
            if field_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Field_GET', "field id must be given",
                    'FIELD ID IS NOT PROVIDED', "FIELD ID")
            handler_ins = APIHandler()
            ...
            apiResponse = APIRequest(handler_ins).get_api_response()
            apiResponse.data = self.get_zcrmfield(
                apiResponse.response_json[APIConstants.FIELDS][0])
        ...
        try:
            if layout_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Layout_GET', "layout id must be given",
                    'LAYOUT ID IS NOT PROVIDED', "LAYOUT ID")
            handler_ins = APIHandler()
            ...
            handler_ins.add_param('module', self.module_instance.api_name)
            api_response = APIRequest(handler_ins).get_api_response()
            api_response.data = self.get_zcrmlayout(
                api_response.response_json[APIConstants.LAYOUTS][0])
    ...
    def get_all_customviews(self):
        try:
            ...
            categories = response_json['info'][
                'translation'] if 'translation' in response_json[
                    'info'] else None
            customviews = response_json[APIConstants.CUSTOM_VIEWS]
            customview_instances = list()

            for customview in customviews:
                customview_instances.append(
                    self.get_zcrm_customview(customview, categories))
        ...
        try:
            if customview_id is None:
                try:
                    ...
                    from Utility import CommonUtil
                CommonUtil.raise_exception(
                    'Customview_GET', "custom view id must be given",
                    'CUSTOM VIEW ID IS NOT PROVIDED', "CUSTOM VIEW ID")
            handler_ins = APIHandler()
            handler_ins.request_url_path = "settings/custom_views/" + \
                str(customview_id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
            ...
            categories = response_json['info'][
                'translation'] if 'translation' in response_json[
                    'info'] else None
            api_response.data = self.get_zcrm_customview(
                response_json[APIConstants.CUSTOM_VIEWS][0], categories)
    ...
    def update_module_settings(self):
        try:
            ...
            handler_ins.request_body = self.construct_json_for_module_update(
    ...
    def update_customview(self, customview_instance):
        try:
            handler_ins = APIHandler()
            handler_ins.request_url_path = "settings/custom_views/" + \
                str(customview_instance.id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_PUT
            handler_ins.add_param('module', self.module_instance.api_name)
            handler_ins.request_body = self.construct_json_for_cv_update(
                customview_instance)
        ...
        try:
            ...
            for related_list in related_lists:
                try:
                    ...
                    from Operations import ZCRMModuleRelatedList
                relatedlist_ins = ZCRMModuleRelatedList.get_instance(
                    related_list['api_name'])
                relatedlist_instances.append(
                    relatedlist_ins.set_relatedlist_properties(related_list))
    ...
    def get_relatedlist(self, relatedlist_id):
        try:
            ...
            relatedlist_ins = ZCRMModuleRelatedList.get_instance(
                related_list['api_name'])
            api_response.data = relatedlist_ins.set_relatedlist_properties(
                related_list)
    ...
    def construct_json_for_module_update(self, module_instance):
        ...
        if module_instance.business_card_fields is not None:
            module_settings[
                'business_card_fields'] = module_instance.business_card_fields
        ...
        if module_instance.related_list_properties is not None:
            prop_arr = dict()
            prop_arr[
                'sort_by'] = module_instance.related_list_properties.sort_by
            prop_arr[
                'sort_order'] = module_instance.related_list_properties.sort_order
    ...
    def get_zcrm_customview(self, customview_details, categories):
        try:
            ...
            from Operations import ZCRMCustomView, ZCRMCustomViewCriteria
        customview_instance = ZCRMCustomView.get_instance(
            self.module_instance.api_name, customview_details['id'])
        ...
        customview_instance.sort_by = customview_details[
            'sort_by'] if 'sort_by' in customview_details else None
        customview_instance.category = customview_details[
            'category'] if 'category' in customview_details else None
        customview_instance.fields = customview_details[
            'fields'] if 'fields' in customview_details else None
        customview_instance.favorite = customview_details[
            'favorite'] if 'favorite' in customview_details else None
        customview_instance.sort_order = customview_details[
            'sort_order'] if 'sort_order' in customview_details else None

        if 'criteria' in customview_details and customview_details[
            ...
            if isinstance(criteria_list, list):
                ...
                for i in range(0, length):
                    ...
                    if criteria == 'or' or criteria == 'and':
                        ...
                        criteria_instance.field = criteria[
                            'field'] if 'field' in criteria else None
                        criteria_instance.value = criteria[
                            'value'] if 'value' in criteria else None
                        criteria_instance.comparator = criteria[
                            'comparator'] if 'comparator' in criteria else None
                ...
                criteria_instance = ZCRMCustomViewCriteria.get_instance()
                criteria_instance.field = criteria_list[
                    'field'] if 'field' in criteria_list else None
                criteria_instance.value = criteria_list[
                    'value'] if 'value' in criteria_list else None
                criteria_instance.comparator = criteria_list[
                    'comparator'] if 'comparator' in criteria_list else None
    ...
    def get_zcrmlayout(self, layout_details):
        ...
        if layout_details['created_by'] is not None:
            layout_instance.created_by = ZCRMUser.get_instance(
                layout_details['created_by']['id'],
                layout_details['created_by']['name'])

        if layout_details['modified_by'] is not None:
            layout_instance.modified_by = ZCRMUser.get_instance(
                layout_details['modified_by']['id'],
                layout_details['modified_by']['name'])

        accessible_profile_arr = layout_details['profiles']
        accessible_profile_instances = list()

        for profile in accessible_profile_arr:
            profile_ins = ZCRMProfile.get_instance(profile['id'],
                                                   profile['name'])
        ...
        layout_instance.sections = self.get_all_sections_of_layout(
            layout_details['sections'])
        ...
        if 'convert_mapping' in layout_details:
            ...
            for convert_module in convert_modules:
                if convert_module in layout_details['convert_mapping']:
                    convert_map = layout_details['convert_mapping'][
                        convert_module]
                    convert_map_ins = ZCRMLeadConvertMapping.get_instance(
                        convert_map['name'], convert_map['id'])

                    if 'fields' in convert_map:
                        field_data = convert_map['fields']

                        for each_field_data in field_data:
                            convert_mapping_field_ins = ZCRMLeadConvertMappingField.get_instance(
                                each_field_data['api_name'],
                                each_field_data['id'])
                            convert_mapping_field_ins.field_label = each_field_data[
                                'field_label']
                            convert_mapping_field_ins.is_required = bool(
                                each_field_data['required'])
                            convert_map_ins.fields.append(
                                convert_mapping_field_ins)
                    layout_instance.convert_mapping[
                        convert_module] = convert_map_ins
    ...
    def get_zcrmfield(self, field_details):
        ...
        field_instance.sequence_number = int(
            field_details['sequence_number']
        ) if 'sequence_number' in field_details else None
        field_instance.id = field_details['id']
        field_instance.is_mandatory = bool(
            field_details['required']) if 'required' in field_details else None
        field_instance.default_value = field_details[
            'default_value'] if 'default_value' in field_details else None
        field_instance.is_custom_field = bool(
            field_details['custom_field']
        ) if 'custom_field' in field_details else None
        field_instance.is_visible = bool(
            field_details['visible']) if 'visible' in field_details else None
        field_instance.field_label = field_details[
            'field_label'] if 'field_label' in field_details else None
        field_instance.length = int(
            field_details['length']) if 'length' in field_details else None
        field_instance.created_source = field_details[
            'created_source'] if 'created_source' in field_details else None
        field_instance.is_read_only = bool(
            field_details['read_only']
        ) if 'read_only' in field_details else None
        field_instance.is_business_card_supported = bool(
            field_details['businesscard_supported']
        ) if 'businesscard_supported' in field_details else None
        field_instance.data_type = field_details[
            'data_type'] if 'data_type' in field_details else None
        field_instance.convert_mapping = field_details[
            'convert_mapping'] if 'convert_mapping' in field_details else None
        ...
        if len(picklist_arr) > 0:
            ...
            for picklist in picklist_arr:
                picklist_instance_arr.append(
                    self.get_picklist_value_instance(picklist))
            field_instance.picklist_values = picklist_instance_arr

        if 'lookup' in field_details and len(field_details['lookup']) > 0:
            field_instance.lookup_field = self.get_lookup_field_instance(
                field_details['lookup'])

        if 'unique' in field_details and len(field_details['unique']) > 0:
            field_instance.is_unique_field = True
            field_instance.is_case_sensitive = bool(
                field_details['unique']['casesensitive'])

        if 'decimal_place' in field_details and field_details[
                'decimal_place'] is not None:
            field_instance.decimal_place = field_details['decimal_place']

        if 'json_type' in field_details and field_details[
        ...
        if 'formula' in field_details and len(field_details['formula']) > 0:
            ...
            field_instance.formula_return_type = field_details['formula'][
                'return_type']
            field_instance.formula_expression = field_details['formula'][
                'expression'] if 'expression' in field_details[
                    'formula'] else None

        if 'currency' in field_details and len(field_details['currency']) > 0:
            field_instance.is_currency_field = True
            field_instance.precision = int(
                field_details['currency']['precision']
            ) if 'precision' in field_details['currency'] else None
            field_instance.rounding_option = field_details['currency'][
                'rounding_option'] if 'rounding_option' in field_details[
                    'currency'] else None

        if 'auto_number' in field_details and len(
                field_details['auto_number']) > 0:
            field_instance.is_auto_number = True
            field_instance.prefix = field_details['auto_number'][
                'prefix'] if 'prefix' in field_details['auto_number'] else None
            field_instance.suffix = field_details['auto_number'][
                'suffix'] if 'suffix' in field_details['auto_number'] else None
            field_instance.start_number = field_details['auto_number'][
                'start_number'] if 'start_number' in field_details[
                    'auto_number'] else None
    ...
    def get_picklist_value_instance(self, picklist):
        ...
        if 'sequence_number' in picklist and picklist[
    ...
    def get_lookup_field_instance(self, lookup_field_details):
        try:
            ...
            from Operations import ZCRMLookupField
        lookup_field_instance = ZCRMLookupField.get_instance(
            lookup_field_details['api_name'])
        lookup_field_instance.display_label = lookup_field_details[
            'display_label']
...
class MetaDataAPIHandler(APIHandler):
    ...
    def get_all_modules(self):
        try:
            ...
            handler_ins.request_api_key = APIConstants.MODULES
            bulk_api_response = APIRequest(handler_ins).get_bulk_api_response()
            modules_json = bulk_api_response.response_json[APIConstants.
                                                           MODULES]
    ...
    def get_zcrmmodule(self, module_details):
        try:
            ...
            from Operations import ZCRMModule
        crmmodule_instance = ZCRMModule.get_instance(
            module_details[APIConstants.API_NAME])
        ...
        crmmodule_instance.web_link = module_details[
            'web_link'] if 'web_link' in module_details else None
        ...
        crmmodule_instance.is_api_supported = bool(
            module_details['api_supported'])
        crmmodule_instance.is_scoring_supported = bool(
            module_details['scoring_supported'])
        crmmodule_instance.module_name = module_details['module_name']
        crmmodule_instance.business_card_field_limit = int(
            module_details['business_card_field_limit']
        ) if 'business_card_field_limit' in module_details else None
        crmmodule_instance.sequence_number = module_details[
            'sequence_number'] if 'sequence_number' in module_details else None
        crmmodule_instance.is_global_search_supported = bool(
            module_details['global_search_supported']
        ) if 'global_search_supported' in module_details else None
        try:
        ...
        if module_details['modified_by'] is not None:
            crmmodule_instance.modified_by = ZCRMUser.get_instance(
                module_details['modified_by']["id"],
                module_details['modified_by']["name"])

        crmmodule_instance.is_custom_module = 'custom' == module_details[
            'generated_type'] if 'generated_type' in module_details else None

        if 'business_card_fields' in module_details:
            crmmodule_instance.business_card_fields = module_details[
                'business_card_fields']

        profiles = module_details['profiles']

        for profile in profiles:
            crmmodule_instance.profiles.append(
                ZCRMProfile.get_instance(profile['id'], profile['name']))

        if 'display_field' in module_details and module_details[
                'display_field'] is not None:
            crmmodule_instance.display_field_name = module_details[
                'display_field']
            # crmmodule_instance.display_field_id=module_details['display_field']['id']) if 'id' in module_details['display_field'] else None

        if 'related_lists' in module_details and module_details[
                'related_lists'] is not None:
            relatedlists = module_details['related_lists']
            relatedlist_instances = list()

            for relatedlist in relatedlists:
                module_relatedlist_ins = ZCRMModuleRelatedList.get_instance(
                    relatedlist['api_name'])
                relatedlist_instances.append(
                    module_relatedlist_ins.set_relatedlist_properties(
                        relatedlist))
            crmmodule_instance.related_lists = relatedlist_instances

        if 'layouts' in module_details and module_details[
                'layouts'] is not None:
            crmmodule_instance.layouts = ModuleAPIHandler.get_instance(
                ZCRMModule.get_instance(
                    module_details[APIConstants.API_NAME])).get_zcrmlayouts(
                        module_details['layouts'])

        if 'fields' in module_details and module_details['fields'] is not None:
            crmmodule_instance.fields = ModuleAPIHandler.get_instance(
                ZCRMModule.get_instance(
                    module_details[APIConstants.API_NAME])).get_zcrmfields(
                        module_details['fields'])

        if 'related_list_properties' in module_details and module_details[
                'related_list_properties'] is not None:
            crmmodule_instance.related_list_properties = self.get_relatedlist_property_instance(
                module_details['related_list_properties'])

        if '$properties' in module_details and module_details[
                '$properties'] is not None:
            crmmodule_instance.properties = module_details['$properties']

        if 'per_page' in module_details and module_details[
                'per_page'] is not None:
            crmmodule_instance.per_page = int(module_details['per_page'])

        if 'search_layout_fields' in module_details and module_details[
                'search_layout_fields'] is not None:
            crmmodule_instance.search_layout_fields = module_details[
                'search_layout_fields']

        if 'custom_view' in module_details and module_details[
                'custom_view'] is not None:
            crmmodule_instance.default_custom_view = ModuleAPIHandler.get_instance(
                ZCRMModule.get_instance(module_details[
                    APIConstants.API_NAME])).get_zcrm_customview(
                        module_details['custom_view'], None)
            crmmodule_instance.default_custom_view_id = module_details[
                'custom_view']['id']

        if 'territory' in module_details and module_details[
                'territory'] is not None:
            crmmodule_instance.default_territory_id = module_details[
                'territory']['id']
            crmmodule_instance.default_territory_name = module_details[
                'territory']['name']
    ...
    def get_relatedlist_property_instance(self, relatedlist_property):
        ...
        reltedlist_property_instance.sort_by = relatedlist_property[
            'sort_by'] if 'sort_by' in relatedlist_property else None
        reltedlist_property_instance.sort_order = relatedlist_property[
            'sort_order'] if 'sort_order' in relatedlist_property else None
        reltedlist_property_instance.fields = relatedlist_property[
            'fields'] if 'fields' in relatedlist_property else None
...
class OrganizationAPIHandler(APIHandler):
    ...
    def get_role(self, role_id):
        try:
            ...
            if role_id is None:
                CommonUtil.raise_exception('Role_GET', "role id must be given",
                                           'ROLE ID IS NOT PROVIDED',
                                           "ROLE ID")
            handler_ins = APIHandler()
    ...
    def get_all_profiles(self):
        try:
            ...
            profiles = bulk_api_response.response_json[handler_ins.
                                                       request_api_key]
    ...
    def get_profile(self, profile_id):
        try:
            ...
            if profile_id is None:
                CommonUtil.raise_exception(
                    'Profile_GET', "profile id must be given",
                    'PROFILE ID IS NOT PROVIDED', "PROFILE ID")
            handler_ins = APIHandler()
            handler_ins.request_url_path = "settings/profiles/" + str(
                profile_id)
            handler_ins.request_method = APIConstants.REQUEST_METHOD_GET
            handler_ins.request_api_key = APIConstants.PROFILES
            api_response = APIRequest(handler_ins).get_api_response()
            profile = api_response.response_json[handler_ins.
                                                 request_api_key][0]
    ...
    def create_user(self, user_instance):
        try:
            ...
            if user_instance is None:
                CommonUtil.raise_exception(
                    'User_POST', "user instance must be given",
                    'USER INSTANCE IS NOT PROVIDED', "USER INSTANCE ID")
            handler_ins = APIHandler()
            handler_ins.request_url_path = "users"
            handler_ins.request_method = APIConstants.REQUEST_METHOD_POST
            handler_ins.request_api_key = APIConstants.USERS
            handler_ins.request_body = self.construct_json_from_user_instance(
                user_instance)
    ...
    def update_user(self, user_instance):
        try:
            ...
            if user_instance is None or user_instance.id is None:
                CommonUtil.raise_exception(
                    'User_PUT', "user instance and id must be given",
                    'USER INSTANCE OR ID IS NOT PROVIDED', "USER INSTANCE ID")
            handler_ins = APIHandler()
    ...
    def delete_user(self, user_id):
        try:
            ...
            if user_id is None:
                CommonUtil.raise_exception(
                    'User_DELETE', "user id must be given",
                    'USER ID IS NOT PROVIDED', "USER ID")
            handler_ins = APIHandler()
    ...
    def get_user(self, user_id):
        try:
            ...
            if user_id is None:
                CommonUtil.raise_exception('User_GET', "user id must be given",
            ...
            handler_ins.request_api_key = APIConstants.USERS
            api_response = APIRequest(handler_ins).get_api_response()
            api_response.data = self.get_zcrm_user(
                api_response.response_json[APIConstants.USERS][0])
    ...
    def construct_json_from_user_instance(self, user_instance):
        ...
        if user_instance.is_personal_account is not None:
            user_info_json['personal_account'] = bool(
                user_instance.is_personal_account)

        if user_instance.default_tab_group is not None:
            user_info_json[
                'default_tab_group'] = user_instance.default_tab_group
        ...
        if user_instance.decimal_separator is not None:
            user_info_json[
                'decimal_separator'] = user_instance.decimal_separator
        ...
        return CommonUtil.create_api_supported_input_json(
            user_info_json, APIConstants.USERS)
    ...
    def get_zcrm_role(self, role_details):
        try:
            ...
            from Operations import ZCRMRole, ZCRMUser
        role_instance = ZCRMRole.get_instance(role_details['id'],
                                              role_details['name'])
        role_instance.display_label = role_details['display_label']
        role_instance.is_admin = bool(role_details['admin_user'])

        if 'reporting_to' in role_details and role_details[
                'reporting_to'] is not None:
            role_instance.reporting_to = ZCRMUser.get_instance(
                role_details['reporting_to']['id'],
                role_details['reporting_to']['name'])
    ...
    def get_zcrm_profile(self, profile_details):
        try:
            ...
            from Operations import ZCRMProfile, ZCRMUser, ZCRMPermission, ZCRMProfileSection, ZCRMProfileCategory
        profile_instance = ZCRMProfile.get_instance(profile_details['id'],
        ...
        if profile_details['modified_by'] is not None:
            profile_instance.modified_by = ZCRMUser.get_instance(
                profile_details['modified_by']['id'],
                profile_details['modified_by']['name'])

        if profile_details['created_by'] is not None:
            profile_instance.created_by = ZCRMUser.get_instance(
                profile_details['created_by']['id'],
                profile_details['created_by']['name'])

        if 'permissions_details' in profile_details:
            permissions = profile_details['permissions_details']

            for permission in permissions:
                permission_ins = ZCRMPermission.get_instance(
        ...
        if 'sections' in profile_details:
            ...
            for section in sections:
                profile_section_instance = ZCRMProfileSection.get_instance(
                    section['name'])
                ...
                if 'categories' in section:
                    ...
                    for category in categories:
                        category_ins = ZCRMProfileCategory.get_instance(
                            category['name'])
                        category_ins.display_label = category['display_label']
                        category_ins.permission_ids = category[
                            'permissions_details']
                        category_ins.module = category[
                            'module'] if 'module' in category else None
                        profile_section_instance.categories.append(
                            category_ins)
    ...
    def get_zcrm_organization(self, org_details):
        try:
            ...
            from Org import ZCRMOrganization
        org_instance = ZCRMOrganization.get_instance(
            org_details['company_name'], org_details['id'])
    ...
    def get_zcrm_user(self, user_details):
        try:
            ...
            from Operations import ZCRMUser, ZCRMRole, ZCRMProfile
        user_instance = ZCRMUser.get_instance(
            user_details['id'],
            user_details['name'] if 'name' in user_details else None)
        user_instance.country = user_details[
            'country'] if 'country' in user_details else None
        user_instance.role = ZCRMRole.get_instance(
            user_details['role']['id'], user_details['role']['name'])

        if 'customize_info' in user_details:
            user_instance.customize_info = self.get_zcrm_user_customizeinfo(
                user_details['customize_info'])
        user_instance.city = user_details['city']
        user_instance.signature = user_details[
            'signature'] if 'signature' in user_details else None
        user_instance.name_format = user_details[
            'name_format'] if 'name_format' in user_details else None
        user_instance.language = user_details['language']
        user_instance.locale = user_details['locale']
        user_instance.is_personal_account = bool(
            user_details['personal_account']
        ) if 'personal_account' in user_details else None
        user_instance.default_tab_group = user_details[
            'default_tab_group'] if 'default_tab_group' in user_details else None
        user_instance.alias = user_details['alias']
        user_instance.street = user_details['street']
        user_instance.city = user_details['city']

        if 'theme' in user_details:
            user_instance.theme = self.get_zcrm_user_theme(
                user_details['theme'])
        ...
        user_instance.decimal_separator = user_details[
            'decimal_separator'] if 'decimal_separator' in user_details else None
        user_instance.website = user_details['website']
        user_instance.time_format = user_details['time_format']
        user_instance.profile = ZCRMProfile.get_instance(
            user_details['profile']['id'], user_details['profile']['name'])
        ...
        try:
            for userkey in user_details:
                if userkey not in ZCRMUser.defaultKeys:
                    user_instance.field_apiname_vs_value[
                        userkey] = user_details[userkey]
    ...
    def get_zcrm_user_customizeinfo(self, customize_info):
        ...
        customize_info_instance.is_to_show_right_panel = bool(
            customize_info['show_right_panel']
        ) if 'show_right_panel' in customize_info else None
        customize_info_instance.is_bc_view = customize_info['bc_view']
        customize_info_instance.is_to_show_home = bool(
            customize_info['show_home']
        ) if 'show_home' in customize_info else None
        customize_info_instance.is_to_show_detail_view = bool(
            customize_info['show_detail_view']
        ) if 'show_detail_view' in customize_info else None
        customize_info_instance.unpin_recent_item = customize_info[
            'unpin_recent_item']
    ...
    def get_zcrm_user_theme(self, user_theme_info):
        ...
        user_theme_instance.normal_tab_font_color = user_theme_info[
            'normal_tab']['font_color']
        user_theme_instance.normal_tab_background = user_theme_info[
            'normal_tab']['background']
        user_theme_instance.selected_tab_font_color = user_theme_info[
            'selected_tab']['font_color']
        user_theme_instance.selected_tab_background = user_theme_info[
