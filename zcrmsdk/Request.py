...
class APIRequest(object):
    ...
    def construct_api_url(self):
        ...
        base_url = ZCRMConfigUtil.get_api_base_url().replace(
            'www', 'sandbox') if hit_sand_box.lower(
            ) == 'true' else ZCRMConfigUtil.get_api_base_url()
    ...
    def authenticate_request(self):
        ...
        if (self.request_headers is None):
            ...
            self.request_headers[
                APIConstants.
                AUTHORIZATION] = APIConstants.OAUTH_HEADER_PREFIX + accessToken
        self.request_headers['User-Agent'] = 'ZohoCRM Python SDK'

    def get_api_response(self):
        try:
            self.authenticate_request()
            connector = HTTPConnector.get_instance(
                self.url, self.request_params, self.request_headers,
                self.request_body, self.request_method, self.request_api_key,
                False)
    ...
    def get_bulk_api_response(self):
        try:
            ...
            return BulkAPIResponse(response, response.status_code, self.url,
                                   self.request_api_key)
        except ZCRMException as ex:
            raise ex
        except Exception as ex:
    ...
    def download_attachment(self):
        try:
            ...
            if response.status_code == APIConstants.RESPONSECODE_OK:
                file_res = FileAPIResponse(response, response.status_code,
                                           self.url)
                ...
                exception = ZCRMException(self.url, response.status_code,
                                          errorMsg, APIConstants.NO_CONTENT,
                                          None, errorMsg)
                raise exception
            else:
