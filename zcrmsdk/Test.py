...
from Operations import (ZCRMCustomView, ZCRMInventoryLineItem,
                        ZCRMJunctionRecord, ZCRMModule, ZCRMNote, ZCRMProfile,
                        ZCRMRecord, ZCRMRole, ZCRMTax, ZCRMUser)
...
class MyClass(object):
    ...
    def create_record(self):
        try:
            ...
            line_item = ZCRMInventoryLineItem.get_instance(
                ZCRMRecord.get_instance("Products", 1386586000000803073))
    ...
    def upload_attachment(self):
        try:
            record = ZCRMRecord.get_instance('Leads', 1386586000001856002)
            resp = record.upload_attachment(
                '/Users/Downloads/sequel-pro-1.1.dmg')
            print resp.data.id
        except ZCRMException as ex:
            print ex.status_code
            print ex.error_message
    ...
    def add_relation(self):
        try:
            record = ZCRMRecord.get_instance('Leads', 1386586000001856002)
            junction_record = ZCRMJunctionRecord.get_instance(
                "Products", 1386586000000803073)
    ...
    def create_records(self):
        try:
            ...
            resp = ZCRMModule.get_instance('Invoices').create_records(
                record_ins_list)
            print resp.status_code
    ...
    def update_records(self):
        try:
            ...
            entityid_list = [
                1386586000001858001, 1386586000001521031, 1386586000001282039
            ]
            bulk_resp = module_ins.update_records(entityid_list, 'Status',
                                                  'Created')
    ...
    def upsert_records(self):
        try:
            ...
            record.add_line_item(line_item)
            record_ins_list.append(record)
            resp = ZCRMModule.get_instance('Invoices').upsert_records(
                record_ins_list)
            print resp.status_code
            entity_responses = resp.bulk_entity_response
    ...
    def delete_records(self):
        try:
            ...
            resp = ZCRMModule.get_instance('Invoices').delete_records(
                entityid_list)
            print resp.status_code
            entity_responses = resp.bulk_entity_response
    ...
    def update_module_settings(self):
        try:
            ...
            module_instance.business_card_fields = [
                'Employees', 'Annual_Revenue', 'Phone', 'Account_Type', 'Fax'
            ]
            module_instance.default_custom_view = ZCRMCustomView.get_instance(
                1386586000000075535, 'All Accounts')
    ...
    def update_customview(self):
        try:
            module_ins = ZCRMModule.get_instance('Accounts')
            cv_ins = ZCRMCustomView.get_instance(1386586000000075535,
                                                 'Accounts')
    ...
    def get_users(self, user_type):
        try:
            if user_type == 'all':
                ...
                resp = ZCRMOrganization.get_instance(
    ...
    def get_user(self):
        try:
            resp = ZCRMOrganization.get_instance().get_user(
                1386586000000105001)
            print resp.status_code
    ...
    def create_user(self):
        try:
            ...
            user_ins.profile = ZCRMProfile.get_instance(
                1386586000000026011, 'Administrator')
    ...
    def update_user(self):
        try:
            user_ins = ZCRMUser.get_instance(1386586000001409001,
                                             'Python Automation user2')
    ...
    def delete_user(self):
        try:
            resp = ZCRMOrganization.get_instance().delete_user(
                1386586000001066023)
            print resp.status_code
            print resp.message
            print resp.code
    ...
    def get_profile(self):
        try:
            resp = ZCRMOrganization.get_instance().get_profile(
                1386586000000026014)
            profiles = [resp.data]
            print resp.status_code
    ...
    def get_role(self):
        try:
            resp = ZCRMOrganization.get_instance().get_role(
                1386586000000026008)
            roles = [resp.data]
            print resp.status_code
